...
namespace JuicyChickenGames
{
    public class ScaleToFitScreen : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Debug.LogWarning($"{nameof(ScaleToFitScreen)} on '{name}' requires a SpriteRenderer with a sprite assigned.", this);
                return;
            }

            if (Camera.main == null)
            {
                Debug.LogWarning($"{nameof(ScaleToFitScreen)} on '{name}' could not find a main camera.", this);
                return;
            }
